...
const footerLinks = [
  { heading: 'కంపెనీ', links: ['మా గురించి', 'బ్లాగులు', 'మమ్మల్ని సంప్రదించండి', 'కెరీర్స్'] },
  { heading: 'కస్టమర్ సేవలు', links: ['నా ఖాతా', 'ఆర్డర్ ట్రాక్ చేయండి', 'తిరిగి పంపడం (రిటర్న్)', 'తరచుగా అడిగే ప్రశ్నలు (FAQ)'] },
  { heading: 'మా సమాచారం', links: ['గోప్యతా విధానం', 'వినియోగదారు నిబంధనలు & షరతులు', 'రిటర్న్ పాలసీ'] },
];

const contactInfo = {
  heading: 'సంప్రదింపు సమాచారం',
  phone: '+0123-456-789',
  email: 'example@gmail.com',
  address: '8502 ప్రెస్టన్ రోడ్, ఇంగిల్‌వుడ్, మైనే 98380',
};

const socialLinks = [
  {
    label: 'ఫేస్‌బుక్', href: '#',
...
// ─── MAIN FOOTER ──────────────────────────────────────────────────────────────
export default function FooterTelugu() {
  return (
      ...
      <div style={{ maxWidth: '1240px', margin: '0 auto', backgroundColor: 'var(--color-white)', borderRadius: 'var(--radius-2xl)', border: '1px solid var(--color-placeholder)', boxShadow: 'var(--shadow-card)', overflow: 'hidden' }}>
        <div className='gap-[2rem] md:gap-[2.5rem]' style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(155px, 1fr))', padding: '2.5rem 2.5rem', position: 'relative' }}>
          <div className='translate-y-[-18px]' style={{ display: 'flex', flexDirection: 'column', gap: '1rem', minWidth: '180px', maxWidth: '240px' }}>
            ...
            <p style={{ color: 'var(--color-muted)', fontSize: '0.8125rem', lineHeight: '1.7', fontFamily: 'var(--font-body)' }}>
              శారీరక మరియు మానసిక ఆరోగ్యం కోసం ఆయుర్వేదం యొక్క పవిత్రత మరియు పురాతన జ్ఞానం. మేము మీ కోసం నేరుగా ప్రకృతి నుండి సేకరించిన అత్యుత్తమ ఉత్పత్తులను అందిస్తున్నాము.
            </p>
          ...
          <div className='absolute left-0 lg:right-0 bottom-0' style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center' }}>
            <img src={paymentImg} alt="అంగీకరించబడే చెల్లింపు పద్ధతులు" style={{ height: '110px', width: 'auto', objectFit: 'contain', objectPosition: 'right center', display: 'block' }} />
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'center', gap: '16px', padding: '1.125rem 2.5rem' }}>
          <p style={{ color: 'var(--color-muted)', fontSize: '0.8125rem', margin: 0, fontFamily: 'var(--font-body)', flexShrink: 0 }}>
            కాపీరైట్ © 2026 <a href="#" style={{ color: 'var(--color-gold)', fontWeight: '600', textDecoration: 'none', transition: 'var(--transition-fast)' }} onMouseEnter={e => { e.currentTarget.style.color = 'var(--color-primary)'; }} onMouseLeave={e => { e.currentTarget.style.color = 'var(--color-gold)'; }}>Vedraha</a>. అన్ని హక్కులూ ప్రత్యేకించబడినవి.
          </p>
        </div>
      </div>
    </footer>
