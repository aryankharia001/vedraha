...
export default function NabhiHeaderTelugu({ onCartOpen, cartCount = 0 }) {
  ...
  const handleLogin = async () => {
    ...
    if (!password) return setError("పాస్‌వర్డ్ అవసరం");
    ...
    try {
      ...
      if (!data.success) return setError(data.message || "లాగిన్ విఫలమైంది");
  ...
  const handleSignup = async () => {
    ...
    if (!validateEmail(email)) return setError("చెల్లుబాటు అయ్యే ఇమెయిల్ చిరునామాను నమోదు చేయండి");
    if (!password) return setError("పాస్‌వర్డ్ అవసరం");
    if (password.length < 6) return setError("పాస్‌వర్డ్ కనీసం 6 అక్షరాలు ఉండాలి");
    if (password !== confirmPass) return setError("పాస్‌వర్డ్‌లు సరిపోలడం లేదు");
    setError(""); setSubmitting(true);
    try {
      ...
      const data = await res.json();
      if (!data.success) return setError(data.message || "నమోదు విఫలమైంది");
      localStorage.setItem("akravi_token", data.token);
      localStorage.setItem("akravi_user", JSON.stringify(data.user));
      setLoggedInUser(data.user);
      setSubmitted(true);
        ...
        <div className="nh-modal">
          <div className="nh-modal-left">
            ...
            <div className="nh-modal-left-sub">
              {mode === "login"
                ? "మీ ఆర్డర్లు, సేవ్ చేసిన వస్తువులు మరియు ప్రత్యేక ఆఫర్లను యాక్సెస్ చేయడానికి లాగిన్ చేయండి."
                : "సభ్యులకు మాత్రమే ఉన్న డీల్స్ మరియు వేగవంతమైన చెక్అవుట్ కోసం మీ ఖాతాను సృష్టించండి."}
            </div>
            ...
            {submitted ? (
              <div className="nh-success-wrap">
                ...
                <div className="nh-success-sub">
                  {mode === "login"
                    ? `మళ్ళీ కలవడం సంతోషంగా ఉంది, ${loggedInUser?.name || loggedInUser?.email?.split("@")[0]}.`
                    : `అక్రవికి స్వాగతం${notify ? ". మేము మీకు అత్యుత్తమ ఆఫర్ల గురించి తెలియజేస్తాము." : "."}`}
                </div>
              ...
              <>
                ...
                <div className="nh-field">
                  ...
                  <div className="nh-input-wrap">
                    <input
                      ...
                      placeholder={mode === "signup" ? "కనీసం 6 అక్షరాలు" : "మీ పాస్‌వర్డ్"}
                      value={password}
                  ...
                  <div className="nh-field">
                    ...
                    <div className="nh-input-wrap">
                      <input
                        ...
                        placeholder="పాస్‌వర్డ్ పునరావృతం చేయండి"
                        value={confirmPass}
                ...
                {mode === "signup" && (
                  <label className="nh-notify-row">
                    ...
                    <span>ఆఫర్లు &amp; అప్‌డేట్లతో నన్ను తెలియజేయండి</span>
                  </label>
                )}
