...
export default function BuyNowModal({ isOpen, onClose, selectedVariant, cartItems, onSuccess }) {
  ...
  const initiateOtp = async () => {
    ...
    try {
      ...
      if (!data?.success) {
        setError(typeof data?.message === "string" ? data.message : "Failed to send OTP. Please try again.");
        return;
      }
  ...
  const verifyOtp = async () => {
    ...
    if (cleanOtp.length < 4) { setError("Please enter a valid OTP."); return; }
    setLoading(true);
    try {
      ...
      const data = await res.json();
      if (!data?.success) {
        setError(typeof data?.message === "string" ? data.message : "OTP verification failed. Please try again.");
        return;
      }
      ...
      setError(e?.message || "Something went wrong during OTP verification.");
    } finally {
      setLoading(false);
    }
  };
    ...
    >
      <div className="bg-white rounded-t-3xl w-full max-w-lg max-h-[90vh] overflow-y-auto pb-8 shadow-2xl flex flex-col">
        ...
        <div className="sticky top-0 bg-white rounded-t-3xl px-5 py-5 flex justify-between items-center border-b border-gray-200 z-10">
          <div>
            <div className="text-2xl font-bold text-gray-900 leading-tight" style={{ fontFamily: "'DM Sans', sans-serif" }}>
              Quick Checkout
            </div>
            <div className="text-xs text-gray-400 mt-0.5">Verify your number to continue</div>
          </div>
          <button
            onClick={onClose}
            disabled={loading}
        ...
        <div className="px-5 pt-6 flex-1">
          {otpStep === "phone" && (
            <div className="bg-gray-900 rounded-xl p-4 mb-5">
              <div className="text-xs uppercase tracking-widest text-gray-500 mb-1">One-Tap Checkout</div>
              <div className="text-base font-bold text-white leading-snug" style={{ fontFamily: "'DM Sans', sans-serif" }}>
                We'll fill in your address <em className="text-[#c8a84b]">automatically</em>
              </div>
              <div className="text-xs text-gray-400 mt-1.5 leading-relaxed">
                Enter your number — no forms to fill, no account needed.
              </div>
            </div>
          )}
          ...
          {otpStep === "phone" && (
            <>
              ...
              <p className="text-xs text-gray-400 text-center leading-relaxed">
                By continuing, you consent to receive an SMS for verification.
              </p>
            </>
          )}
          ...
          {otpStep === "otp" && (
            <>
              <div className="mb-3.5 text-sm text-gray-500 leading-relaxed">
                OTP sent to <strong className="text-gray-900">+{countryCode} {phone}</strong>
              </div>
              <div className="mb-4">
