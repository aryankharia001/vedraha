// src/components/shared/ProductPage.jsx  (updated)
// ─────────────────────────────────────────────────────────────────────────────
// Changes from original:
//  • Remove local cartItems / cartOpen state — use CartContext instead
//  • Remove CartDrawer render here (it lives globally in App.jsx now)
//  • Remove NabhiHeader render here (it lives globally in App.jsx now)
//  • handleAddToCart now calls context.addToCart + context.openCart
// ─────────────────────────────────────────────────────────────────────────────
...
import { useCart } from "../../../components/CartContext";   // ← NEW
...
// CartDrawer is no longer rendered here — it's global in App.jsx
...
export default function ProductPage({ config, relatedProducts = [] }) {
  ...
  const handleAddToCart = () => {
    ...
    addToCart(product, v, quantity);   // ← writes to global context
    openCart();                         // ← opens global CartDrawer
  };
  ...
  const openShiprocketGateway = async (clickEvent, checkoutItems) => {
    ...
    try {
      await window.HeadlessCheckout.addToCart(checkoutItems);
    } catch (err) {
  ...
  const handleBuyNowDirect = (e) => {
    trackInitiateCheckout(product.id, product.name, selectedPrice, quantity);
    const v = variants.find((vv) => vv.id === selectedVariant);
    openShiprocketGateway(e, [{ variantId: String(v.externalVariantId), quantity }]);
  };
  ...
  const handleCartBuyNow = (e) => {
    ...
    openShiprocketGateway(e, checkoutItems);
  };

  const handleCodSubmit = async (formData) => {
    setCodSubmitting(true);
    try {
      const fbc  = getCookie("_fbc")  || "";
      const fbp  = getCookie("_fbp")  || "";
      const ttclid = getCookie("ttclid") || getCookie("_ttp") || "";
      await axios.post(`${backendurl}/api/orders/cod`, {
        ...formData,
  ...
  return (
    <div style={{ background: "#fff", minHeight: "100vh" }}>
      ...
      {/* COD modal only — CartDrawer is now global in App.jsx */}
      ...
      {/* NOTE: NabhiHeader is now rendered globally in App.jsx — removed from here */}
          ...
          <div className="animate-fadeIn">
            ...
            <div className="flex items-center gap-4 mb-5">
              ...
              <div className="flex items-center border border-gray-300 rounded-xl overflow-hidden bg-white w-fit">
                ...
                >−</button>
                <div className="w-12 text-center text-base font-bold text-gray-900 border-l border-r border-gray-200 h-10 flex items-center justify-center" style={{ fontFamily: "'DM Sans', sans-serif" }}>
                  {quantity}
                </div>
                <button
              ...
              >
                <div className="w-full rounded-xl flex items-center justify-center gap-2" style={{ background: themeColor, height: "48px" }}>
                  <ShoppingBag size={18} color="#ffffff" />
                  <span className="text-sm font-bold text-white tracking-wide">Add To Cart</span>
                  {cartTotalQty > 0 && (
                    <span className="bg-white/25 rounded-full px-2 py-0.5 text-xs font-bold text-white">{cartTotalQty}</span>
                  )}
                </div>
              </button>
            </div>
            ...
            <div className="flex flex-col md:flex-row gap-2.5 mt-5">
              ...
              >
                <div className="w-full rounded-xl flex items-center justify-center gap-2" style={{ background: themeColor, height: "48px" }}>
                  <span className="text-sm font-bold text-white tracking-wide">Cash On Delivery</span>
                  {cartTotalQty > 0 && (
                    <span className="bg-white/25 rounded-full px-2 py-0.5 text-xs font-bold text-white">{cartTotalQty}</span>
                  )}
                </div>
              ...
              <button
                ...
                style={{ WebkitTapHighlightColor: "transparent", fontFamily: "'DM Sans', sans-serif" }}
              >
                <div className="w-full rounded-xl flex items-center justify-center gap-2" style={{ background: "rgb(184, 148, 84)", height: "48px" }}>
                  <UpiStack icons={upiIcons} size="sm" />
                  <span className="text-sm font-bold text-white tracking-wide">Buy Now</span>
                </div>
              </button>
            </div>
