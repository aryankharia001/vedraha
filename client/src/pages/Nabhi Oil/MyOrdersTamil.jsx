...
const STATUS_META = {
  pending:    { label: "நிலுவையில் உள்ளது",  color: "text-amber-600",  bg: "bg-amber-50",   border: "border-amber-200",  icon: <Clock size={12} /> },
  confirmed:  { label: "உறுதிப்படுத்தப்பட்டது", color: "text-emerald-700", bg: "bg-emerald-50",  border: "border-emerald-200", icon: <CheckCircle size={12} /> },
  processing: { label: "செயலாக்கத்தில் உள்ளது", color: "text-blue-600",   bg: "bg-blue-50",    border: "border-blue-200",   icon: <Package size={12} /> },
  shipped:    { label: "அனுப்பப்பட்டது",    color: "text-violet-600", bg: "bg-violet-50",  border: "border-violet-200", icon: <Truck size={12} /> },
  delivered:  { label: "வழங்கப்பட்டது",     color: "text-green-700",  bg: "bg-green-50",   border: "border-green-200",  icon: <BadgeCheck size={12} /> },
  cancelled:  { label: "ரத்து செய்யப்பட்டது", color: "text-red-600",    bg: "bg-red-50",     border: "border-red-200",    icon: <XCircle size={12} /> },
};

const PAY_META = {
  COD:                { label: "பண வழங்கல் (COD)", color: "text-amber-600",  bg: "bg-amber-50"  },
  "Cash on Delivery": { label: "பண வழங்கல் (COD)", color: "text-amber-600",  bg: "bg-amber-50"  },
  cod:                { label: "பண வழங்கல் (COD)", color: "text-amber-600",  bg: "bg-amber-50"  },
    ...
    <div className="bg-white/80 backdrop-blur-sm border border-stone-200/50 rounded-2xl overflow-hidden shadow-xl shadow-stone-200/50 mb-8">
      ...
      <div className={`overflow-hidden transition-all duration-500 ease-out ${open ? "max-h-[800px] opacity-100" : "max-h-0 opacity-0"}`}>
        <div className="px-6 py-8 bg-gradient-to-b from-stone-50/30 to-white">
          <p className="text-[11px] font-bold text-stone-400 tracking-widest uppercase mb-6">தனிப்பட்ட தகவல்</p>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-6">
            <div className="space-y-2">
              <label className="block text-[11px] font-bold text-stone-400 tracking-widest uppercase">முழு பெயர்</label>
              <input className={inputBase} type="text" placeholder="உங்கள் பெயர்" value={name}
                onChange={(e) => { setName(e.target.value); clearMsg(); }} />
            </div>
            <div className="space-y-2">
              <label className="block text-[11px] font-bold text-stone-400 tracking-widest uppercase">மின்னஞ்சல் முகவரி</label>
          ...
          <p className="text-[11px] font-bold text-stone-400 tracking-widest uppercase mb-6">
            கடவுச்சொல் மாற்று{" "}
            <span className="font-normal normal-case tracking-normal opacity-60">(விரும்பினால்)</span>
          </p>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <div className="sm:col-span-2 space-y-2">
              <label className="block text-[11px] font-bold text-stone-400 tracking-widest uppercase">தற்போதைய கடவுச்சொல்</label>
            ...
            <div className="space-y-2">
              <label className="block text-[11px] font-bold text-stone-400 tracking-widest uppercase">புதிய கடவுச்சொல்</label>
              <div className="relative">
            ...
            <div className="space-y-2">
              <label className="block text-[11px] font-bold text-stone-400 tracking-widest uppercase">புதிய கடவுச்சொல் உறுதிப்படுத்து</label>
              <div className="relative">
                <input
    ...
    <div className="min-h-screen bg-gradient-to-br from-stone-100 via-green-50/30 to-amber-50/30 pb-24">
      ...
      {gatewayLoading && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[99999] flex flex-col items-center justify-center gap-5 animate-fade-in">
          ...
          <span className="text-white text-base font-semibold tracking-wide">செக்அவுட் திறக்கிறது…</span>
        </div>
      )}
      ...
      <div className="w-full max-w-7xl mx-auto px-4 sm:px-8 lg:px-12 pt-10">
        ...
        {loading && orders.length === 0 && (
          <div className="flex flex-col items-center justify-center mt-24 gap-4">
            ...
            <p className="text-sm text-stone-400">உங்கள் ஆர்டர்கள் ஏற்றப்படுகின்றன…</p>
          </div>
        )}
          ...
          {!loading && orders.length === 0 && (
            <AnimatedSection delay={200} className="col-span-full">
              <div className="flex flex-col items-center justify-center py-28 px-8 bg-white/60 backdrop-blur-sm rounded-3xl border border-stone-200/50 shadow-xl">
                ...
                <h2 className="font-display text-2xl font-bold text-gray-900 mb-3">இன்னும் ஆர்டர்கள் இல்லை</h2>
                ...
                >
                  நாபி அம்ருத் வாங்க <ArrowRight size={16} />
                </button>
              </div>
            </AnimatedSection>
          )}
              ...
              >
                ...
                <div className="flex items-start justify-between gap-4 px-5 py-5">
                  ...
                  <div className="flex flex-col items-end gap-2">
                    ...
                    <div className="flex items-center gap-1 text-stone-400 group-hover:text-green-800 transition-colors">
                      <span className="text-xs font-medium">விவரங்கள் பார்</span>
                ...
                <div className="px-5 py-3 bg-gradient-to-r from-stone-50/50 to-transparent border-t border-stone-100/50">
                  <div className="flex items-center gap-2 text-xs text-stone-400">
                    <Package size={12} />
                    <span>ஆர்டர் விவரங்கள் மற்றும் டிராக்கிங் பார்க்க கிளிக் செய்யவும்</span>
                  </div>
                </div>
              </div>
            );
        ...
        {orders.length > 0 && (
          <AnimatedSection delay={500}>
            <div className="flex items-center justify-center gap-2 mt-10 text-xs text-stone-400">
              ...
              <span>இந்த சாதனத்தில் சேமிக்கப்பட்ட ஆர்டர்கள் · நிலை புதுப்பிப்புகள் நிர்வாக மாற்றங்களை நேரடியாக பிரதிபலிக்கும்</span>
            </div>
          </AnimatedSection>
        )}
      </div>
