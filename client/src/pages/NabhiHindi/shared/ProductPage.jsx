    ...
    >
      <div className="flex flex-col gap-0.5">
        ...
        <div className="flex items-center gap-1">
          <span className="text-xs text-red-500 font-bold">🔥 जल्दी खरीदें!</span>
      ...
      <button onClick={onBuyNow} className="p-0 border-0 rounded-xl cursor-pointer bg-transparent outline-none flex-shrink-0" style={{ WebkitTapHighlightColor: "transparent" }}>
        <div className="bg-gradient-to-br from-gray-900 to-black rounded-xl">
          <div className="bg-gradient-to-br from-gray-800 to-gray-900 rounded-[10px] h-12 px-8 flex items-center justify-center gap-2">
            ...
            <span className="text-sm font-extrabold text-white whitespace-nowrap">अभी खरीदें</span>
          </div>
        </div>
      </button>
    </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12 pb-12">
          <div>
            <div className="flex items-center gap-2.5 mb-3">
              ...
              <span className="text-white text-xl font-bold" style={{ fontFamily: "'Noto Sans Devanagari', sans-serif" }}>{product.name}</span>
            </div>
            <p className="text-gray-500 text-sm leading-relaxed mb-6 max-w-[280px]">आधुनिक वेलनेस के लिए प्राचीन आयुर्वेदिक ज्ञान।</p>
          </div>
          <div>
            <div className="text-white text-base font-bold mb-5" style={{ fontFamily: "'Noto Sans Devanagari', sans-serif" }}>त्वरित लिंक</div>
            {[{ label: "होम", target: "home" }, { label: "हमारे बारे में", target: "about" }, { label: "समीक्षाएं", target: "reviews" }, { label: "संपर्क करें", target: "contact" }].map((l) => (
              <a key={l.label} href={`#${l.target}`} className="block text-gray-500 text-sm mb-3 no-underline hover:text-white">{l.label}</a>
            ))}
          </div>
          <div>
            <div className="text-white text-base font-bold mb-5" style={{ fontFamily: "'Noto Sans Devanagari', sans-serif" }}>नीतियां</div>
            {["गोपनीयता नीति", "वापसी नीति", "शिपिंग नीति", "सेवा की शर्तें"].map((l) => (
              <a key={l} href="#" className="block text-gray-500 text-sm mb-3 no-underline hover:text-white">{l}</a>
            ))}
          </div>
          <div>
            <div className="text-white text-base font-bold mb-5" style={{ fontFamily: "'Noto Sans Devanagari', sans-serif" }}>संपर्क</div>
  ...
  const openShiprocketGateway = async (clickEvent, checkoutItems) => {
    ...
    try {
      ...
      const response = await axios.post(
        ...
        { items: checkoutItems.map((i) => ({ variant_id: i.variantId, quantity: i.quantity })), redirect_url: `${window.location.origin}/exc-payment-success-hn${queryString ? `?${queryString}` : ""}`, paramsObject },
        { headers: { "Content-Type": "application/json" } }
      );
      const token = response.data?.result?.token;
      if (!token) throw new Error("No token");
      window.HeadlessCheckout.addToCart(clickEvent, token, { fallbackUrl: `${window.location.origin}/payment-faliure` });
    } catch (err) {
      console.error(err);
    ...
    <div className="font-sans text-gray-900 bg-white min-h-screen">
      ...
      {gatewayLoading && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", zIndex: 99999, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 16 }}>
          ...
          <span style={{ color: "#fff", fontSize: 16, fontWeight: 600 }}>चेकआउट खुल रहा है…</span>
      ...
      <NabhiHeaderHindi onCartOpen={() => setCartOpen(true)} cartCount={cartTotalQty} />
      ...
      <div className="max-w-[1100px] mx-auto px-5" id="home">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-0 md:gap-12 py-5 md:py-10">
          ...
          <div className="animate-fadeIn">
            ...
            <h1 className="text-2xl md:text-3xl font-bold m-0 mb-2.5 leading-tight tracking-tight" style={{ fontFamily: "'Noto Sans Devanagari', sans-serif" }}>
              {product.h1}
            </h1>
            ...
            <div className="text-xs font-bold uppercase tracking-widest mb-3.5 text-gray-400">पैक चुनें</div>
            {variants.map((v) => (
              <div
                key={v.id}
