...
const SORT_OPTIONS = [
  { value: "newest", label: "नए उत्पाद" },
  { value: "price_asc", label: "मूल्य: कम से अधिक" },
  { value: "price_desc", label: "मूल्य: अधिक से कम" },
  { value: "discount", label: "सर्वश्रेष्ठ छूट" },
];
  ...
  return (
    <div className="exc-card" onClick={() => navigate(redirectUrl)}>
      ...
      <div className="exc-card__img-wrap">
        {discountPct > 0 && (
          <div className="exc-card__badge">{discountPct}% छूट</div>
        )}
        <img
          src={product.image}
          alt={product.name}
          ...
          onError={(e) => {
            ...
            e.target.parentElement.innerHTML = `
              <div style="width:100%;height:100%;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;background:#EAF5EE;">
                ...
                <span style="font-size:11px;color:#97C9A8;font-family:sans-serif;">चित्र उपलब्ध नहीं</span>
              </div>
            `;
          }}
        />
      ...
      <div className="exc-card__body">
        ...
        <div className="exc-card__price-row">
          ...
          {/* {discountPct > 0 && (
            <span className="exc-card__discount">−{discountPct}%</span>
          )} */}
        </div>

        {/* <button
          className={`exc-card__btn${added ? " added" : ""}`}
          onClick={handleAddToCart}
          disabled={adding}
        >
          {adding ? (
            <span className="exc-btn-spinner" />
          ) : added ? (
            <>
...
// ─── Main Catalog ─────────────────────────────────────────────────────────────
export default function ExclusiveProductCatalogHindi() {
 ...
 useEffect(() => {
  ...
  const openShiprocketGateway = async (clickEvent, checkoutItems) => {
    if (!window.HeadlessCheckout?.addToCart) {
      alert("चेकआउट लोड हो रहा है। कृपया एक क्षण बाद पुनः प्रयास करें।");
      return;
    }
  ...
  useEffect(() => {
    ...
    fetch(`${backendurl}/api/exclusiveproducts`)
      ...
      .catch(() => { setError("उत्पाद लोड करने में विफल।"); setLoading(false); });
  }, []);

  useEffect(() => {
    let r = [...products].filter((p) => {
      const price = p.discountedPrice ?? p.price ?? 0;
      return p.lang === "hi" && price >= minPrice && price <= maxPrice;
    });
    switch (sort) {
    ...
    <div style={{ fontFamily: "'DM Sans', sans-serif", background: "#F4FAF6", minHeight: "100vh", paddingBottom: 60 }}>
      ...
      {gatewayLoading && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", zIndex: 99999, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 16 }}>
          ...
          <span style={{ color: "#fff", fontSize: 16, fontWeight: 600, fontFamily: "'DM Sans', sans-serif" }}>चेकआउट खुल रहा है…</span>
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        /* thumb-only variant used in dual slider */
        .exc-range--thumb {
      ...
      <div style={{ background: "#fff", borderBottom: "1px solid #D1E8D8", padding: "36px 20px 28px" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          ...
          <p style={{ fontSize: 13, color: "#6B9E80", margin: 0, lineHeight: 1.7, fontFamily: "'DM Sans', sans-serif" }}>
            असली परिणामों के लिए तैयार किए गए प्रीमियम आयुर्वेदिक फॉर्मूले
          </p>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "24px 20px 0" }}>
        ...
        {filterOpen && (
          <div className="exc-filter-panel">
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 18 }}>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 10, color: "#A8C4B0", fontWeight: 600, marginBottom: 5, fontFamily: "'DM Sans', sans-serif", letterSpacing: "0.08em", textTransform: "uppercase" }}>न्यूनतम</div>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 10, color: "#A8C4B0", fontWeight: 600, marginBottom: 5, fontFamily: "'DM Sans', sans-serif", letterSpacing: "0.08em", textTransform: "uppercase" }}>अधिकतम</div>
                <div style={{ position: "relative" }}>
                  <span style={{ position: "absolute", left: 10, top: "50%", transform: "translateY(-50%)", fontSize: 13, color: "#6B9E80", fontWeight: 700, fontFamily: "'DM Sans', sans-serif" }}>₹</span>
                  <input
                    type="number"
            ...
            {/* Dual-handle track (visual only — two overlapping range inputs) */}
        ...
        {hasFilters && (
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 16, alignItems: "center" }}>
            <span style={{ fontSize: 11, color: "#6B9E80" }}>सक्रिय फ़िल्टर:</span>
        ...
        {!loading && error && (
          <div style={{ textAlign: "center", padding: "80px 0" }}>
            ...
            <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 22, fontWeight: 700, color: "#18181B", marginBottom: 8 }}>कुछ गलत हो गया</div>
        ...
        {!loading && !error && filtered.length === 0 && (
          <div style={{ textAlign: "center", padding: "80px 0" }}>
            ...
            <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 22, fontWeight: 700, color: "#18181B", marginBottom: 8 }}>कोई उत्पाद नहीं मिला</div>
            <div style={{ color: "#6B9E80", fontSize: 13, marginBottom: 20 }}>अपने फ़िल्टर बदलकर देखें</div>
            {hasFilters && (
